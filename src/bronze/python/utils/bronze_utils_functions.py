...
def get_max_timestamp_for_table(spark_session: SparkSession, table_name:str,env:str, pipeline: str = "entidades") -> str:
    try:
        query = f"SELECT NVL(max(last_execution_ts),'1900-01-01 00:00:00') as ts FROM bronze{env}.audit.pipeline_run_ctrl WHERE table_name = '{table_name}' and pipeline = '{pipeline}'"
        logging.info(query)
