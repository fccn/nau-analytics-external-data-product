...
def delta_load(spark: SparkSession, jdbc_url:str, MYSQL_USER:str, MYSQL_SECRET:str,last_updated:str,table:str) -> Tuple[bool, str]:
    ...
    incremental_df = add_ingestion_metadata_column(df=new_df,table=table)
    saveTale = f"bronze_local.entidades.{table}"
...
def main() -> None:
    ...
    ICEBERG_DATABASE_CATALOG_NAME = get_required_env("ICEBERG_DATABASE_CATALOG_NAME")
    ...
    ICEBERG_CATALOG_URI = f"jdbc:mysql://{ICEBERG_CATALOG_HOST}:{ICEBERG_CATALOG_PORT}/{ICEBERG_DATABASE_CATALOG_NAME}"
