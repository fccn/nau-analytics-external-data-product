...
def main():
    ...
    spark = start_iceberg_session("silver_auth_userprofile")
    ...
    df_src_data = spark.sql(f"""
        SELECT id,
                ...
                phone_number,
                state,
                get_json_object(meta, '$.employment_situation') AS employment_situation,
                SHA1(
                    CONCAT_WS('||',
                        name,
                        year_of_birth,
                        ...
                        phone_number,
                        state,
                        get_json_object(meta, '$.employment_situation')
                        )
                    ) AS row_hash,
                ingestion_date
